            ...
            if (instructions_with_pcoffset.find(opcode) != instructions_with_pcoffset.end())
            {
                switch (opcode)
                {
                    case 13:
                        ...
                        program_counter += value_or_offset;
                        break;
                    case 15:
                        if (reg_A == 0)
                            program_counter += value_or_offset;
            ...
            else
            {
                switch (opcode)
                {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        if (stack_pointer + value_or_offset < 0 || stack_pointer + value_or_offset > MAX_SIZE)
                        {
                            if (options == "-trace")
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        if (reg_A + value_or_offset < 0 || reg_A + value_or_offset > MAX_SIZE)
                        {
                            if (options == "-trace")
                                cout << "ERROR: Invalid Memory Access\n";
                            return;
...
// Convert 24bit string to decimal appropriately
int string_to_decimal_24(string s)
{
    int value = 0;
